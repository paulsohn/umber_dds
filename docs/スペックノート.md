...
+ Stateful References Implementation:
...
discovery protocolの役割は発見されたremote Endpointに情報を提供することである。Participantがその情報を自身が持つEndpointを設定するためにどのように使われるかは、実際のRTPS protocolの実装に依存し、discovery protocolの仕様の一部ではない。例えば、8.4.7で紹介されているreferences実装では、remote Endpointで獲得した情報は以下を設定することを可能にする。
...
DataReaders to reliable RTPS StatefulReaders. 実際の実装ではstatefull references 実装を使う必要はない。
...
どのようにEndpointが設定されるかはプロトコルの実装に依存する。stateful references 実装では、この操作は以下のようなlogical stepsで行われる。
```
